...
const EmojiGuesses: React.FC<EmojiGuessesProps> = ({ guesses, gameOver, gameWon }) => {
	...
	const emojis = useMemo(() => {
		const filteredGuesses = guesses.map((guess) => guess.filter((attribute) => attribute.name !== "image"));
			...
			{emojis.map((guess, index) => (
				<div key={index} className="flex flex-row justify-center">
					{guess.map((emoji, innerIndex) => (
						<div key={innerIndex} className="text-4xl">
							{emoji}
						</div>
					))}
				</div>
