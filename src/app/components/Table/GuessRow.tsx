...
const GuessRow: React.FC<GuessRowProps> = ({ guess, thresholds, target_guess, rowIndex }) => {
		...
		for (const column of columns) {
			...
			if (distance !== undefined) {
				rendered_columns.push(
					<Tile key={`${rowIndex}-${column.name}`} comparison={distance} attribute={column} isNumericTile={true} />
				);
			} else {
				rendered_columns.push(<Tile key={`${rowIndex}-${column.name}`} attribute={column} isNumericTile={false} />);
			}
		}
	...
	function getHeightDistance(currentNumber: string) {
		const height_data = EvaluateNumericalGuess(
			currentNumber,
			target_guess.height,
			thresholds.height.high,
			thresholds.height.very_high
		);
		return height_data.comparison;
	}

	function getCodeDistance(currentNumber: string) {
		const code_data = EvaluateNumericalGuess(
			currentNumber,
			target_guess.code,
			thresholds.code.high,
			thresholds.code.very_high
		);
	...
	return (
		<>
			...
			<tr key={rowIndex} className="inline-flex flex-row">
				<Tile key={`${rowIndex}-image`} attribute={image} isImageTile={true} isEmpty={image.value === ""} />

				<Tile key={`${rowIndex}-name`} attribute={name} width="wide" />
